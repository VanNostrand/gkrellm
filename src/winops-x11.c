	...
	*/
	...
	if (cp->top_spacer.pixmap)
		...
		if (cp->transparency == 2 && cp->top_spacer.mask)
			...
			gkrellm_draw_drawable(cp->top_spacer.pixmap,
	...
	if (cp->bottom_spacer.pixmap)
		...
		if (cp->transparency == 2 && cp->bottom_spacer.mask)
			...
			gkrellm_draw_drawable(cp->bottom_spacer.pixmap,
...
gkrellm_winop_apply_rootpixmap_transparency(void)
	...
	depth_ret = 0;
	gdk_window_get_geometry(gtk_widget_get_window(top_window), NULL, NULL, NULL, NULL, &depth_visual);
